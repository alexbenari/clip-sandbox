...
int run_probe(const std::filesystem::path& movie, const std::filesystem::path& cache_path,
    ...
    std::cout << "{\"type\":\"source\",\"track\":" << source.GetTrack()
              ...
              << "},\"constructorMs\":" << std::fixed << std::setprecision(3) << constructor_ms
               ...
               << ",\"hardwareDevice\":" << frame_review::JsonString(hardware_device).serialized() << "}\n";
...
int wmain(int argc, wchar_t** argv) {
    ...
    try {
        ...
        std::cout << "{\"type\":\"error\",\"mode\":"
                  << frame_review::JsonString(frame_review::Utf8Text(mode).value()).serialized()
                  << ",\"message\":" << frame_review::JsonString(error.what()).serialized() << "}\n";
