...
export class MediabunnyPanel {
  ...
  mount(host: HTMLElement): void {
    ...
    this.root.innerHTML = `
      <section class="candidate-panel">
        <header class="candidate-panel__header">
          <div>
            <h2>Candidate B</h2>
            <p>Custom Mediabunny / CanvasSink control</p>
          </div>
    ...
    for (const button of rateButtons) {
      button.addEventListener('click', () => {
        this.callbacks.onRateChange(Number(button.dataset.rate) as PlaybackRate);
      });
    }
  }
  ...
  getCanvas(): HTMLCanvasElement {
    ...
    if (!canvas) {
      throw new Error('Candidate B canvas is not mounted.');
    }
    return canvas;
  }
