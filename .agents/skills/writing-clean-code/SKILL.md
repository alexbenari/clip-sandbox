...
7. **Comment only what the code cannot say.** *(Henney, 97/17.)* A comment that restates what the code does adds nothing. A comment that contradicts the code is worse than nothing — wrong comments survive forever because no compiler catches them. The legitimate space is *why this approach, not what it does*. House rule: comments are rare, phrased in simple language, and at most two short sentences; never reference external documents, specs, calibration records, decision dates, or people — state the constraint itself, in place.
   *Check:* every comment in the hunk explains *why*, not *what*, in simple language, in at most two short sentences with no external references; comments that describe the code itself are deleted, and the underlying name or extraction is improved instead.
