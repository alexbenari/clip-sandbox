...
GIF capture, refinement, original-source extraction, extraction-publication refresh, and coordinated shutdown ownership updated: 2026-09-21. Reusable frame-review player and generic fixed/contextual screen integration updated: 2026-09-20. Prepared-review host, persistent cache reuse, renderer-safe service, and native frame-review build ownership updated: 2026-09-14. Shell ownership, utilities, coordinated panels, grid layout, durable settings, startup workspace selection, production TypeScript object ownership, and interface naming updated: 2026-09-21. Pipeline catalog folder grant boundary verified: 2026-09-26.
...
8. `folderSession` in [`src/adapters/electron/electron-file-system-service.ts`](../../src/adapters/electron/electron-file-system-service.ts): direct-access desktop folder session backed by an absolute folder path. [`FolderAccessRegistry`](../../electron/folder-access-registry.cjs) records picker-selected paths and explicitly opened catalog pipelines as per-window folder grants; listing or expanding the catalog grants only opaque read access to its entries. Only a granted folder session can write, append, delete, or re-enumerate a folder through Electron IPC.
...
20. `AppSettings` in [`src/app/app-settings.ts`](../../src/app/app-settings.ts) and [`AppSettingsService`](../../src/app/app-settings-service.ts): persisted renderer preferences, including `startupScreenId`, its default/fallback (`gif-extraction`), and committed immutable settings state. The service owns load/save; it does not own the active screen.
...
Movie opening has three progressive capabilities. Dedicated frame-review movie selection registers the original source with the window's `FrameReviewHost` and returns an opaque handle; ordinary pipeline files and `ClipFileSnapshot` do not retain that session capability. LibVLC opens the selected original source first for provisional playback with audio. `ReviewPreparationService` then inspects the source and validates a content- and tool-version-bound cache identity. On a cold miss it conditionally normalizes timestamps and creates/publishes the software GOP1 review proxy first; `ReviewSession` switches LibVLC to that proxy as soon as it is usable, so timestamp scrubbing no longer waits for exact-frame work. It then builds the canonical BestSource index, indexes the proxy, validates the ordinal frame map, and publishes the complete prepared review. Exact capture remains disabled until that final prepared review is activated. Indexing emits real native progress when the gate supplies totals; other phases are deliberately indeterminate. After exact readiness, frame-scrub delivery carries canonical identity and pixels together; capture during clocked playback remains a timestamp/inexact point.

`PlaybackProxyCache` owns early proxy publication and one process-wide writer per identity; its immutable proxy and optional normalized source live under `<app-folder>/playback-proxy-cache`. `ExactReviewProxyCache` retains atomic publication for the same proxy together with its exact-review set: canonical indexes live under `<app-folder>/frame-index-cache`, while proxy indexes, frame maps, hard-linked proxy/normalized media, and manifests live under `<app-folder>/exact-review-proxy-cache`. Each requesting session is an independent consumer: cancelling one consumer does not cancel a writer still needed by another, while abandoning the final consumer aborts the writer and removes its partial workspace. The application folder is the project directory in development and the installed executable's containing directory in a packaged build, never an `app.asar` path. Valid full-review hits skip both builders and preserve mtimes across session and process restarts. Temporary partial entries are ignored and cleaned, stale or incomplete entries rebuild, and ordinary session/app shutdown never deletes validated cache assets.
...
`index.html` defines the global app bar, central command host and main screen host. [`ApplicationShellController`](../../src/ui/application-shell-controller.ts) owns a nonempty set of [`IAppScreen`](../../src/ui/app-screen.ts) registrations, the persistent workspace selector, active content/commands and delegated initial focus. Screen roots stay mounted but inactive roots are hidden/inert; inactive commands are detached. Activation is synchronous, so there are no queued stale completion callbacks. The selector is hidden for a single registration and lives in the global app bar, not in the movie/media workspace. The shell may receive an explicit initial screen id, but it has no screen-id branches or Electron/filesystem dependencies.

Production registers [`CollectionScreen`](../../src/ui/collection-screen.ts), [`GifExtractionScreen`](../../src/ui/gif-extraction-screen.ts), contextual [`RefineGifScreen`](../../src/ui/refine-gif-screen.ts), and [`SettingsScreen`](../../src/ui/settings-screen.ts). Fixed screens receive persistent selector entries; a contextual screen receives a temporary truthful entry only while active. Collection adapts the existing Collection DOM, owns initial focus and supplies the grid's allocated content height. `MainToolbarControl` remains the Collection command-state renderer; its name and stable element ids are retained. Existing Activity and the prominent Workspace selector live in the global bar, while movie controls, zoom/context overlays and screen-specific commands remain in their own workspace areas. Settings has no command bar and exposes the persisted Startup workspace preference. The Pipelines catalog uses a persistent fallback host inside the shell panel mount, so screen activation preserves its tree and disclosure state. Collection keyboard handlers run only while Collection is active; [`GifWorkflowKeyboardController`](../../src/ui/gif-workflow-keyboard-controller.ts) is activated and released by the two GIF screen lifecycles.

[`AppSettingsService`](../../src/app/app-settings-service.ts) owns the committed immutable [`IAppSettings`](../../src/app/app-settings.ts) value: an optional Pipelines root, the default audio preference for new single-clip playback (off initially), and the persisted `startupScreenId` preference (GIF Extraction by default). Settings owns edit/busy/feedback UI; the [`Electron adapter`](../../src/adapters/electron/electron-app-settings-service.ts) validates unknown IPC responses and the domain parser owns valid startup-workspace membership. [`AppSettingsStore`](../../electron/app-settings-store.cjs) reads and atomically writes versionless known fields in `app-settings.json` beneath Electron userData. It ignores unrecognized persisted fields and returns an Activity-visible diagnostic; invalid or unreadable known settings fall back to defaults. Failed writes preserve the committed value. Native folder cancellation saves nothing. `AppController` resolves the loaded startup preference and activates the shell with it; `ApplicationShellController` owns the active screen after that handoff, not the persisted preference. After settings load, [`PipelineCatalogRuntime`](../../electron/pipeline-catalog-runtime.cjs) reads only immediate child folders of the configured root, keeps folders containing at least one recognized top-level video, and returns opaque per-window catalog ids and names; `extraction-tmp` sorts first and the remaining names sort naturally. [`PipelineCatalogControl`](../../src/ui/pipeline-catalog-control.ts) owns the side-panel disclosure tree: expanding a pipeline lazily reveals distinct Collections and Clips groups, and expanding a collection reveals its listed clip names. Load is a separate action that obtains the full active folder session through the catalog grant; it uses the same unsaved-collection guard as Browse Folder, while catalog discovery never mutates the active `PipelineSession`. Changing the root reloads this temporary filesystem catalog; a future durable pipeline registry may replace its provider without changing the panel contract. The trusted extraction runtime also reads the root only to resolve the fixed `extraction-tmp` system pipeline. Zoom samples the audio preference for each new video; its local toggle stays local and the grid remains muted. See [`pipeline-catalog-runtime.spec.ts`](../../tests/unit/pipeline-catalog-runtime.spec.ts), [`pipeline-catalog-control.spec.ts`](../../tests/unit/pipeline-catalog-control.spec.ts), and [`settings.spec.ts`](../../tests/e2e/settings.spec.ts) for coverage.
...
8. flows that durably add files, such as exact extraction, publish an explicit completion event rather than watching the filesystem; `OpenFolderRefreshSession` re-enumerates an already-open matching clean folder, or defers the refresh until its active Collection save succeeds.
